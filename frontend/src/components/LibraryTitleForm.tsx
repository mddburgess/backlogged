    ...
    {({ values }) => (
      <FormikForm>
        ...
        <FieldArray name="copies">
          {({ push, remove }) => (
            <div>
              {values.copies.length > 0 &&
                values.copies.map((copy, index) => (
                  <Form.Row>
                    <Col>
                      <Form.Group controlId={`copies[${index}].platform`}>
                        <Form.Label>Platform</Form.Label>
                        <Field name={`copies[${index}].platform`} as={Form.Control} />
                      </Form.Group>
                    </Col>
                    <Col>
                      <Form.Group controlId={`copies[${index}].service`}>
                        <Form.Label>Service</Form.Label>
                        <Field name={`copies[${index}].service`} as={Form.Control} />
                      </Form.Group>
                    </Col>
                    <Col xs="auto">
              ...
              <Button onClick={() => push({ platform: "", service: "" })}>Add Copy</Button>
            </div>
          )}
        </FieldArray>
        <div>
